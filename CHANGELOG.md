...
### Added
- **✅ ЗАВЕРШЁН Task016: Диагностика отдачи 1С и прохода фильтра дефицита:**
  - **Расширенный debug-эндпойнт:** добавлен анализ тестовых артикулов AV-04362, AV-04172, AV-04964 с автоматическим поиском в полях `sku`, `article`, `art`, `Артикул` и в названиях товаров
  - **Конфигурируемый фильтр дефицита:** добавлен параметр `MIN_DEFICIT` (по умолчанию 1) и возможность обхода фильтрации через `bypass_filter=true` в триггере пополнения
  - **Улучшенное поле-маппинг 1С:** расширена поддержка полей дефицита (`need_to_order`, `quantity_to_order`, `КоличествоКЗаказу`) и артикулов с автоматическим извлечением SKU
  - **Детализированное логирование фильтрации:** добавлены события `replenishment.filter_input`, `replenishment.filter_output`, `replenishment.filter_summary` с режимами normal/bypass и подробной статистикой причин отбраковки
  - **Безопасность debug-функций:** маскировка чувствительных заголовков (Authorization, password, token) в debug-выводе, защита токеном `DEBUG_ONEC_TOKEN`
- **✅ ЗАВЕРШЁН Task013: Debug-эндпойнт и улучшенная телеметрия пополнения:**
  - **Debug-эндпойнт `/api/v1/debug/onec/deficit`:** прямой прокси к 1С API для получения сырых ответов без фильтрации, защищенный токеном
  - **Настройки debug-режима:** добавлены `DEBUG_ONEC_TOKEN`, `ONEC_BASE_URL`, `LOG_ONEC_RAW` в конфигурацию приложения
  - **Детализированная телеметрия:** улучшено логирование в `replenishment_service.py` с подробными причинами отбраковок элементов
  - **Структурированные логи фильтрации:** новые события `replenishment.fetch_deficit.raw` и `replenishment.filter_summary` с полной трассировкой обработки данных
  - **Безопасность:** debug-функциональность доступна только с корректным заголовком X-Debug-Token

### Fixed
- **✅ ЗАВЕРШЁН Task015: Улучшения debug-функциональности и диагностики фильтрации:**
  - **Добавлен параметр message в log_event:** функция `log_event()` теперь принимает опциональный параметр `message` для обратной совместимости с существующим кодом
  - **Нормализация ответов 1С:** debug-эндпойнт теперь корректно обрабатывает различные форматы ответов 1С (`{"#value": [...]}`, `{"value": [...]}`, прямые массивы) и возвращает структурированную информацию с подсчетом элементов
  - **Трассировка фильтрации:** добавлены события `replenishment.filter_input` и `replenishment.filter_output` для полной диагностики обработки данных до и после фильтрации
  - **Улучшенная диагностика:** теперь можно точно определить, пусты ли данные от 1С изначально или отбраковываются нашими фильтрами, с подробными причинами отклонения каждого элемента
- **✅ ЗАВЕРШЁН Task014: Исправления debug-эндпойнта для корректной работы:**
  - **Добавлена аутентификация в 1С API:** debug-эндпойнт теперь использует те же креденшелы (`API_1C_USER`, `API_1C_PASSWORD`), что и основное приложение, исправлена ошибка 401 Unauthorized
  - **Исправлен вызов log_event:** убран несуществующий параметр `message`, информация теперь передается через `details.note`
  - **Подтверждена корректность логирования:** debug-запросы корректно записываются в `integration_logs` с полной информацией о статусе ответа и содержимом
