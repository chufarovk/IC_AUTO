...
### Fixed
- **Исправлена админ-панель:** устранены ошибки подключения к приложению и совместимости с PostgreSQL
  - Исправлен URL для межконтейнерных обращений: изменен с `http://app:8000` на `http://app` (внутренний порт 80)
  - Заменен MySQL-стиль SQL `INTERVAL :mins MINUTE` на PostgreSQL-совместимый `make_interval(mins => :mins)`
  - Добавлены переменные окружения для админ-панели: `ADMIN_APP_URL`, `ADMIN_DB_URL`, `ADMIN_POLL_SECONDS`, `ADMIN_PAGE_SIZE`
  - Обновлена конфигурация docker-compose для корректного проброса переменных окружения в контейнер admin_panel
  - Расширена функциональность админ-панели: добавлены фильтры по времени, уровню, системе, статусу и шагам
  - Улучшен интерфейс: добавлена информация о подключениях к БД и приложению в заголовке панели
