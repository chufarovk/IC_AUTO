...
### `migrations/`
Управление схемой базы данных через Alembic migrations.

- **`env.py`**: Настройка Alembic окружения с следующими улучшениями (Task008):
  - Автоматическое чтение `DATABASE_URL` из переменных окружения
  - Конвертация asyncpg → psycopg2 для совместимости с синхронным Alembic
  - Функция `_alembic_url()` для безопасной работы с драйверами БД
- **`versions/`**: Файлы миграций, включая merge-миграции для разрешения multi-head конфликтов
  - Все основные таблицы: `integration_logs`, `outbox_events`, `pending_transfers`
  - Поддержка полей наблюдаемости в integration_logs
...
- **`prestart.py`** — скрипт предварительной инициализации, выполняемый перед запуском приложения:
  - Проверка подключения к базе данных с ожиданием готовности
  - Применение миграций Alembic с конвертацией asyncpg → psycopg2 для совместимости
  - Emergency bootstrap: автоматическое создание таблицы `integration_logs` через raw DDL при сбое миграций
  - Обеспечение устойчивого запуска приложения независимо от состояния БД

- **`start.sh`** — bash-скрипт запуска приложения в продакшене:
  - Вызов `prestart.py` для инициализации БД и миграций
  - Запуск Uvicorn на порту 80 внутри контейнера (мапится наружу как 8000:80)

- **Database Bootstrap (Task008)** — гарантированное создание критических таблиц:
  - Автоматическое создание `integration_logs`, `outbox_events`, `pending_transfers` через DDL при сбоях миграций
  - Применение `alembic stamp head` для синхронизации состояния после ручного создания таблиц
  - Устранение ошибок "relation does not exist" для APScheduler и админ-панели
  - Поддержка всех полей наблюдаемости в integration_logs согласно спецификации
