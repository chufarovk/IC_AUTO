...
- **`app/api/`**: Слой Маршрутизации (Routing Layer).
  ...
  - `debug_onec.py` — debug-эндпойнты для получения сырых ответов 1С без обработки, защищенные токеном. Включает полную аутентификацию в 1С API, нормализацию различных форматов ответов 1С (`{"#value": [...]}`, `{"value": [...]}`, прямые массивы), и структурированное логирование с подсчетом элементов.
...
- **`app/services/`**: Слой Бизнес-логики (Business Logic Layer).
  ...
  - `logger_service.py` — запись событий в таблицу `integration_logs` с поддержкой новых полей наблюдаемости и функция `log_event()` для стандартизированного логирования. Поддерживает опциональный параметр `message` для обратной совместимости с legacy-кодом.
  - `replenishment_service.py` — переписан с пошаговым логированием, декораторами `@log_step`, структурированными событиями в БД и детализированной телеметрией фильтрации с причинами отбраковок. Добавлена трассировка фильтров (`replenishment.filter_input`/`replenishment.filter_output`) для полной диагностики обработки данных.
...
- **`app/core/`**: Ядро Приложения.
  ...
  - `config.py` — настройки приложения через Pydantic Settings, включая debug-конфигурацию (DEBUG_ONEC_TOKEN, ONEC_BASE_URL, LOG_ONEC_RAW).
...
- В `app/api/v1/endpoints/` добавлены:
  - `replenishment.py` — эндпоинт-триггер `POST /api/v1/trigger/internal-replenishment` с поддержкой bypass_filter для диагностики.
  - `admin.py` — административные эндпоинты для инициализации таблиц: `POST /admin/init_logs_table`.
- В `app/api/` добавлены:
  - `debug_onec.py` — диагностические эндпоинты для анализа ответов 1С: `GET /api/v1/debug/onec/deficit` с детальным логированием, поиском тестовых артикулов (AV-04362, AV-04172, AV-04964) и маскировкой чувствительных данных.
...
- Безопасный рендер логов с типизированными проверками (Task009)
- Функция `fetch_logs_safe()` для предотвращения падений при сравнении DataFrame со строкой
